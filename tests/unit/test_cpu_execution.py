...
def test_execute_addi():
    cpu = CPU()
    cpu.registers[1] = 10
    cpu.execute_arithmetic(
        ["R3", "R1", "7"], op_func=operator.add, op_name="ADDI", immediate=True
    )
...
def test_execute_lw_misaligned_offset():
    ...
    with pytest.raises(Exception, match="Invalid offset"):
        cpu.execute_memory_action(
            ["R2", "3(R1)"], "LW"
        )  # offset = 3 (not divisible by WORD_SIZE)
...
def test_execute_instructions_add_and_halt():
    ...
    cpu.load_instructions = [["ADD", "R1", "R2", "R3"], ["HALT"]]
...
def test_execute_instructions_with_error():
    ...
    cpu.load_instructions = [["ADD", "R1", "R2", "R3"], ["INVALID_OP", "R1", "R2"]]
